$(document).ready(function(){
    if (document.getElementById('container_travels')) {
      // var url = 
      $.post(amigable("?module=shop&function=show_travels"), function(data,status) {
        // console.log(data);
        ...
        $.each(json, function(index, list) {
              ...
              ElementDiv.innerHTML = "<div id='prueba' class='block-4 text-center border'>"
                                      ...
                                      + "<a class='details' id="+ list.id + "><img class='img-fluid' src=http://localhost/www/FW_PHP_OO_MVC_JQUERY/Andiamo/view/assets/images/" + list.img + "></a>"
                                      + "</figure>" 
                                      + "<div class='block-4-text p-4'>"
                                      + "<h3><a class='details' id="+ list.id + ">" + list.destination + "</a></h3>"
    ...
    if (document.getElementById('container_travels2')) {
      $.post(amigable("?module=shop&function=show_travels2"), function(data,status) {
        var json = JSON.parse(data);
        $.each(json, function(index, list) {
          // console.log(list.id);
              var ElementDiv = document.createElement('div');
    ...
    if (document.getElementById('details')) {
      $.post(amigable("?module=shop&function=view_shop_details"), function(data,status) {
        var json = JSON.parse(data);
        console.log(json);
        $.each(json, function(index, list) {
        ...
        var ElementDiv = document.createElement('div');
              ...
              ElementDiv.innerHTML = "<div class='col-md-6'>"
                                    + "<img src='http://localhost/www/FW_PHP_OO_MVC_JQUERY/Andiamo/view/assets/images/" + list.img + "' alt='Image' class='img-fluid'>"
                                    + "</div>"
                                    + "<div class='col-md-6'>"
                                    + "<h2 class='text-black'>"+ list.destination +"</h2>"
                                    + "<p>" + list.country + "</p>"
                                    + "<p class='mb-4'>Ex numquam veritatis debitis minima quo error quam eos dolorum quidem perferendis. Quos repellat dignissimos minus, eveniet nam voluptatibus molestias omnis reiciendis perspiciatis illum hic magni iste, velit aperiam quis.</p>"
                                    + "<p><strong class='icon icon-shopping_cart text-primary h4'>" + list.price + "€</strong></p>"
                                    + "<span id='" + list.id + "' class='InsertCarr icon icon-shopping_cart'></span>"
                                    + "</div>"
  ...
  $(document).on('click','.details',function () {
    ...
    console.log(id);
    // $.post(amigable("?module=shop&function=details"), {"id":id}, function(data,status) {
    //   window.location.href = amigable("?module=shop&function=details");
    //   console.log(data);
    // });
    // window.location.href = 'index.php?page=controller_shop&op=details&id=' + id;
    window.location.href = amigable("?module=shop&function=details&aux=" + id);
  });
});